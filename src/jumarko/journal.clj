...
#_(println (.plot (ASCIIGraph/fromSeries (double-array czech-deaths))))
...
(defn log ^double [^double x]
  ;; no need to use anything fancy like Neanderthal at this point:
  (Math/log x))
(println (.plot (ASCIIGraph/fromSeries (double-array (map log czech-deaths)))))

;; check also confirmed cases, not just deaths
(def czech-confirmed (country-observations "Czechia" (confirmed world-data)))
(import '(com.mitchtalmadge.asciidata.graph ASCIIGraph))
(println (.plot (ASCIIGraph/fromSeries (double-array (map log czech-confirmed)))))
